...
def test_elementwise_product():
    ...
    assert calculate_elementwise_product(A, B).tolist() == [
        [0.5, 1.0, 1.5],
        [2.0, 2.5, 3.0],
    ], "wrong output"
...
def test_product():
    ...
    assert calculate_matrix_product(X, W).tolist() == [[3.0]], "wrong output"
...
def test_neuron():
    ...
    assert isinstance(
        calculate_matrix_prod_with_bias(X, W, b), torch.Tensor
    ), "not a tensor"

    assert calculate_activation(torch.tensor(0.0)) == torch.tensor(
        0.0
    ), "wrong activation"
    assert calculate_activation(torch.tensor(0.1)) == torch.tensor(
        1.0
    ), "wrong activation"
    assert calculate_activation(torch.tensor(-0.1)) == torch.tensor(
        0.0
    ...
    assert isinstance(
        calculate_activation(torch.tensor(0.0)), torch.Tensor
    ), "not a tensor"
...
def test_a_batch_of_inputs():
    ...
    assert calculate_matrix_prod_with_bias(X, W, b).tolist() == [
        [1.5],
        [6.0],
    ], "wrong output"
...
def test_two_neurons_with_a_batch_of_inputs():
    ...
    assert calculate_output(X, W, b).tolist() == [
        [1.0, 1.0],
        [1.0, 1.0],
    ], "wrong output"
